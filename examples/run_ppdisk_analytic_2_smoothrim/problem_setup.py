...
def grid_refine_inner_edge(x_orig,nlev,nspan):
    ...
    return x
#
# A smoothing function for the inner rim
#
def smooth_gauss_inner_edge(r,sigma,factor,drr):
    dlgw   = drr/np.sqrt(-2*np.log(factor))
    dlgr   = np.log(r/r[0])-drr
    dlgr[dlgr>0] = 0.
    smooth = np.exp(-0.5*(dlgr/dlgw)**2)
    return sigma*smooth
#
...
#
# Now smooth the inner rim
#
sm_fact  = 1e-2
sm_drr   = 0.1
sigmad   = smooth_gauss_inner_edge(r,sigmad,sm_fact,sm_drr)  # Smooth the inner rim
