...
# Grid parameters
#
nx       = 100       # Basic grid only. Refinement will add points
ny       = 20        # One-sided only. So the "real" value is twice this.
nz       = 1
#
# Model parameters
#
...
xi       = grid_refine_inner_edge(xi,nlev_r,nspan_r)   # Refinement at inner edge
yi       = math.pi/2.0 - zmaxr*np.linspace(ny,0,ny+1)/(ny*1.0)
zi       = np.array([0.,math.pi*2])
xc       = 0.5e0 * ( xi[:-1] + xi[1:] )
yc       = 0.5e0 * ( yi[:-1] + yi[1:] )
nx       = len(xc)     # Recompute nx, because of refinement at inner edge
#
